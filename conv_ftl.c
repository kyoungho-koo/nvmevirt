...
#ifdef FDP_SIMULATOR
static inline bool fdp_last_pg_in_wordline(struct fdp_ftl *fdp_ftl, struct ppa *ppa)
{
	struct ssdparams *spp = &fdp_ftl->ssd->sp;
	return (ppa->g.pg % spp->pgs_per_oneshotpg) == (spp->pgs_per_oneshotpg - 1);
}

static bool fdp_should_gc(struct fdp_ftl *fdp_ftl, int rg_id)
{
	struct reclaim_group_mgmt *rgm = &(fdp_ftl->rgm[rg_id]);
	int free_ru_cnt = rgm->free_ru_cnt;

	if (free_ru_cnt <= fdp_ftl->fp.gc_thres_ru) {
		NVMEV_INFO("%s: execute garbage collection! rg_id %d free_ru_cnt %d gc_thres_ru %d rgm->vpc %d rgm->ipc %d\n", 
				__func__, rg_id, free_ru_cnt, fdp_ftl->fp.gc_thres_ru, rgm->vpc, rgm->ipc);
	}

	return free_ru_cnt <= fdp_ftl->fp.gc_thres_ru;
}

static bool fdp_should_gc_high(struct fdp_ftl *fdp_ftl, int rg_id)
{
	struct reclaim_group_mgmt *rgm = &(fdp_ftl->rgm[rg_id]);
	int free_ru_cnt = rgm->free_ru_cnt;

	if (free_ru_cnt <= fdp_ftl->fp.gc_thres_ru_high) {
		NVMEV_INFO("%s: execute garbage collection! rg_id %d free_ru_cnt %d gc_thres_ru_high %d rgm->vpc %d rgm->ipc %d\n", 
				__func__, rg_id, free_ru_cnt, fdp_ftl->fp.gc_thres_ru_high, rgm->vpc, rgm->ipc);
	}

	return free_ru_cnt <= fdp_ftl->fp.gc_thres_ru_high;
}

static int fdp_check_victim_ru_ipc(struct fdp_ftl *fdp_ftl, int phnd_id, int rg_id)
{
	struct reclaim_group_mgmt *rgm = &(fdp_ftl->rgm[rg_id]);
	int free_ru_cnt = rgm->free_ru_cnt;
	...
	return victim_ru->ipc;
}

static inline struct ppa fdp_get_maptbl_ent(struct fdp_ftl *fdp_ftl, uint64_t lpn)
{
	NVMEV_ASSERT(fdp_ftl != NULL);
	/*
	NVMEV_INFO("%s: fdp_ftl %d fdp_ftl->maptbl %p \n", 
			__func__, fdp_ftl->id, fdp_ftl->maptbl);
	*/
	return fdp_ftl->maptbl[lpn];
}

static inline void fdp_set_maptbl_ent(struct fdp_ftl *fdp_ftl, uint64_t lpn, struct ppa *ppa)
{
	NVMEV_ASSERT(lpn < fdp_ftl->ssd->sp.tt_pgs);
	fdp_ftl->maptbl[lpn] = *ppa;
}

static uint64_t fdp_ppa2pgidx(struct fdp_ftl *fdp_ftl, struct ppa *ppa)
{
	struct ssdparams *spp = &fdp_ftl->ssd->sp;
	uint64_t pgidx;
	...
	NVMEV_INFO("%s: ch:%d, lun:%d, pl:%d, blk:%d, pg:%d\n", 
			__func__, ppa->g.ch, ppa->g.lun, ppa->g.pl, ppa->g.blk, ppa->g.pg);
			*/

	/*
	NVMEV_INFO("%s: pgs_per_ch: %d pgs_per_lun: %d pgs_per_pl: %d pgs_per_blk: %d \n", 
			__func__, spp->pgs_per_ch, spp->pgs_per_lun, spp->pgs_per_pl, spp->pgs_per_blk);
	*/
...
static inline uint64_t fdp_get_rmap_ent(struct fdp_ftl *fdp_ftl, struct ppa *ppa)
{
	uint64_t pgidx = fdp_ppa2pgidx(fdp_ftl, ppa);

	return fdp_ftl->rmap[pgidx];
}

/* set rmap[page_no(ppa)] -> lpn */
static inline void fdp_set_rmap_ent(struct fdp_ftl *fdp_ftl, uint64_t lpn, struct ppa *ppa)
{
	uint64_t pgidx = fdp_ppa2pgidx(fdp_ftl, ppa);
...
static void fdp_per_reclaim_group_gc(struct fdp_ftl *fdp_ftl, int phnd_id, int rg_id);
	...
	if (ruh_wfc->write_credits <= 0) {
		...
		for (i = 0; i < 4; i++) {
			...
			for (rg_id = 0; rg_id < RG_PER_FTL; rg_id++) {
				int new_ipc = fdp_check_victim_ru_ipc(fdp_ftl, ruh_id, (rg_id + i) % RG_PER_FTL);
				if (max_ipc < new_ipc) {
					max_ipc = new_ipc;
					select_rg_id = (rg_id + i) % RG_PER_FTL;
				}
			}
			fdp_per_reclaim_group_gc(fdp_ftl, ruh_id, select_rg_id);
			ruh_wfc->write_credits += ruh_wfc->credits_to_refill;
			wfc->write_credits += wfc->credits_to_refill;
		}
...
static void init_fdp_lines_for_lun(struct fdp_ftl *fdp_ftl, uint32_t ch, uint32_t lun) 
{
	...
	//lm->lines = kmalloc(sizeof(struct line) * lm->tt_lines, GFP_KERNEL);
	//
	NVMEV_ASSERT(lm->lines != NULL);
	//NVMEV_ASSERT(sizeof(struct line) * lm->tt_lines == vmalloc_size(lm->lines));

	/*
	NVMEV_INFO("%s: fdp_ftl %d ch %d lun %d  fdp_ftl %p lm %p lm->lines %p\n", 
				__func__, fdp_ftl->id, ch, lun, fdp_ftl, lm, lm->lines);
				*/
...
static void prepare_ru_wp_for_lun(struct fdp_ftl *fdp_ftl, struct reclaim_unit_handle *ruh, struct reclaim_unit *ru) {
	...
	*wp = (struct write_pointer){
	...
	curline->rup = ru;
	/*
	NVMEV_INFO("%s: (fdp_ftl %d phnd %d ru_idx %d) line %p line->id %d ru->id %d ru->ch %d ru->lun %d\n", 
			__func__, fdp_ftl->id, ru->ruh_id, ru->rg_id, 
			curline, curline->id, ru->id, ru->ch, ru->lun);
			*/
	ru->ulc++;
}
...
static struct reclaim_unit *get_next_free_ru(struct fdp_ftl *fdp_ftl, struct reclaim_unit_handle *ruh, uint16_t rg_id, int from)
{
	struct reclaim_group_mgmt *rgm = &fdp_ftl->rgm[rg_id];
	...
	if (rgm->free_ru_cnt < 16) {
		NVMEV_INFO("%s: RU Not Enough fdp_ftl %d write_credits %d ruh->id %d ruh->ru_idx %d ruh->gc_ru_idx %d rg_id %d from %d rgm->free_ru_cnt %d rgm->victim_ru_cnt[%d] %d rgm->full_ru_cnt %d rgm->ref_cnt %d\n", 
				__func__, fdp_ftl->id, fdp_ftl->wfc.write_credits, ruh->id, ruh->ru_idx, ruh->gc_ru_idx, rg_id, from, rgm->free_ru_cnt, ruh->id, rgm->victim_ru_cnt[ruh->id], rgm->full_ru_cnt, rgm->ref_cnt);
	}

	if (!cur_ru) {
		NVMEV_ERROR("No free RU left in VIRT !!!! rgm->free_ru_cnt %d\n", rgm->free_ru_cnt);
		return NULL;
	}

	INIT_LIST_HEAD(&cur_ru->ru_line_list);

	cur_ru->ruh_id = ruh->id;
	if (cur_ru->ref_cnt != 0) {
		NVMEV_INFO("%s: ref_cnt is not zero fdp_ftl %d write_credits %d ruh->id %d ruh->ru_idx %d ruh->gc_ru_idx %d rg_id %d from %d rgm: free_ru_cnt %d victim_ru_cnt %d full_ru_cnt %d ref_cnt %d cur_ru: id %d ref_cnt %d is_victim %d\n", 
				__func__, fdp_ftl->id, fdp_ftl->wfc.write_credits, ruh->id, ruh->ru_idx, ruh->gc_ru_idx, rg_id, from, rgm->free_ru_cnt, rgm->victim_ru_cnt[ruh->id], rgm->full_ru_cnt, rgm->ref_cnt, cur_ru->id, cur_ru->ref_cnt, cur_ru->is_victim);
	}
	NVMEV_ASSERT(cur_ru->ref_cnt == 0);
	cur_ru->ref_cnt++;
	rgm->ref_cnt++;
	...
	/* fdp_v2 : per Channel */
	prepare_ru_wp_for_lun(fdp_ftl, ruh, cur_ru);
...
static struct line *get_next_free_line_for_channel(struct fdp_ftl *fdp_ftl, uint32_t target_channel)
{
	...
	if (!curline) {
		NVMEV_ERROR("No free line left for channel %d\n", target_channel);
		return NULL;
	}
...
{
	struct reclaim_unit *ru = NULL;
	NVMEV_ASSERT(ruh != NULL);
	if (io_type == USER_IO) {
		ru = ruh->ru[ruh->ru_idx];
	} else if (io_type == GC_IO) {
		ru = ruh->gc_ru[ruh->gc_ru_idx];
	}

	/*
	if (io_type == GC_IO) {
		if (ru == NULL) {
			NVMEV_ERROR("%s: ruh->id %d ruh->ru_idx %d ruh->gc_ru_idx %d io_type %d\n", 
					__func__, ruh->id, ruh->ru_idx, ruh->gc_ru_idx, io_type);
		} else {
			NVMEV_INFO("%s: ruh->id %d ruh->ru_idx %d ruh->gc_ru_idx %d io_type %d\n", 
					__func__, ruh->id, ruh->ru_idx, ruh->gc_ru_idx, io_type);
		}
	}
	*/
...
static struct write_pointer *__get_ftl_wp(struct fdp_ftl *fdp_ftl, uint16_t phnd_id , uint32_t io_type)
{
	...
	struct line *ru_line = list_first_entry_or_null(&ru->ru_line_list, 
				struct line, entry);

	/*
	NVMEV_INFO("%s: (fdp_ftl %d phnd %d ru_idx %d) ru %p ru->id %d ru->rg_id %d ru->ch %d ru->lun %d line %p line->id %d wp->ch %d wp->lun %d wp->pg %d wp->blk %d wp->pl %d\n", 
			__func__, fdp_ftl->id, phnd_id, ruh->ru_idx, 
			ru, ru->id, ru->rg_id, ru->ch, ru->lun, 
			ru_line, ru_line->id,
...
//skip_ru:
	...
	struct reclaim_group_mgmt *rgm = &fdp_ftl->rgm[rup->rg_id];
	...
	if (rup->ulc != spp->lines_per_ru) {
		...
		/*
		NVMEV_INFO("%s: rg_id %d rup->pgs %d rup->ulc %d\n", 
					__func__, *ru_idx, rup->pgs, rup->ulc);
		*/

		list_add_tail(&wpp->curline->entry, &rup->ru_line_list);
		rup->ulc++;
		wpp->curline->rup = rup;
	...
	NVMEV_ASSERT(rup->ref_cnt == 1);
	rup->ref_cnt--;
	rgm->ref_cnt--;
	if (rup->vpc == spp->pgs_per_ru) {
		/* all pgs are still valid, move to full line list */
		NVMEV_ASSERT(rup->ipc == 0);
		list_add_tail(&rup->entry, &rgm->full_ru_list);
		rgm->full_ru_cnt++;
		//NVMEV_INFO("RU: move ru to full_ru_list rup->ipc %d rup->vpc %d rg_id %d full_ru_cnt %d\n", rup->ipc, rup->vpc, rgm->id, rgm->full_ru_cnt);
		//NVMEV_INFO("[NoFreeLine] %s() full_line_cnt++ %d\n", __func__, lm->full_line_cnt);
	} else {
		...
		// NVMEV_INFO("[NoFreeLine] %s() victim_ru_cnt++ %d\n", __func__, rgm->victim_ru_cnt);
	}
...
*/
	...
	//*ru_idx = (*ru_idx + 1) % RG_PER_FTL;
	//if (io_type == GC_IO) {
	// 	NVMEV_INFO("%s: GC_IO ru %p gc_ru_idx %d *ru_idx %d\n", __func__, *rupp,ruh->gc_ru_idx, *ru_idx);
	//}

	/*
	 * Barrow from another Reclaim Group
	if (*rupp == NULL) {
		*ru_idx = (*ru_idx + 1) % RG_PER_FTL;
		if (free_check == *ru_idx) {
			goto out;
		}
		goto retry;
	}
...
static void advance_fdp_write_pointer(struct fdp_ftl *fdp_ftl, uint32_t phnd_id, uint32_t io_type)
{
	...
	*/
	...
	rup = __get_ftl_ru(fdp_ftl, phnd_id, io_type);
	wpp = &rup->wp;

out:
	/*
	if (io_type == GC_IO) {
		NVMEV_INFO("advanced wpp: fdp_ftl->id %d phnd_id %d ch:%d, lun:%d, pl:%d, blk:%d, pg:%d (curline %d)\n",
					fdp_ftl->id, phnd_id, wpp->ch, wpp->lun, wpp->pl, wpp->blk, wpp->pg, wpp->curline->id);
	}
	*/
	rup->pgs++;
}
...
static void remove_fdp_placement(struct fdp_ftl *fdp_ftl)
{
	...
	if (phndls == NULL) {
		NVMEV_INFO("%s: placement handle list is NULL\n", __func__);
		return;
	}
	...
	for (p_idx = 0; p_idx < phndls->nphndls; p_idx++) {
		...
		if (ruh == NULL) {
			NVMEV_INFO("%s: reclaim unit handle is NULL\n", __func__);
			continue;
		}

		for (rg_idx = 0; rg_idx < RG_PER_FTL; rg_idx++) {
			struct reclaim_unit *ru = ruh->ru[rg_idx];

			if (ru == NULL) {
				NVMEV_INFO("%s: reclaim unit is NULL\n", __func__);
				continue;
			}
...
static void fdp_init_ftl(struct fdp_ftl *fdp_ftl, int ftl_id, struct fdpparams *fpp, struct ssd *ssd)
{
	...
	/* initialize all the lines for fdp_v1 */
...
static void fdp_init_params(struct fdpparams *fpp)
{
	...
	fpp->gc_thres_ru = 64; /* Need only two lines.(host write, gc)*/
...
static inline struct reclaim_group_mgmt *get_rgm(struct fdp_ftl *fdp_ftl, struct ppa *ppa)
{
	struct reclaim_unit *ru;
	// line = get_line_per_channel(fdp_ftl, ppa);
	ru = get_ru(fdp_ftl, ppa);

	return &(fdp_ftl->rgm[ru->rg_id]);
}
#endif //FDP_SIMULATOR
...
#ifdef FDP_SIMULATOR
static void fdp_mark_page_valid(struct fdp_ftl *fdp_ftl, struct ppa *ppa, int where, int phnd_id, uint32_t io_type)
{
	struct ssdparams *spp = &fdp_ftl->ssd->sp;
	struct nand_block *blk = NULL;
	struct nand_page *pg = NULL;
	...
	struct reclaim_unit_handle *ruh = __get_ftl_ruh(fdp_ftl, phnd_id);
	struct reclaim_unit *ru;
	struct reclaim_group_mgmt *rgm;

	/* update page status */
	pg = get_pg(fdp_ftl->ssd, ppa);
	if(pg->status != PG_FREE) {
		NVMEV_ERROR("%s: pg is not free where %d \n", __func__, where);
	}
	NVMEV_ASSERT(pg->status == PG_FREE);
	pg->status = PG_VALID;
	...
	if (ru->vpc < 0 || ru->vpc >= spp->pgs_per_ru) {
		struct line *ru_line = list_first_entry_or_null(&ru->ru_line_list, struct line, entry);

		NVMEV_INFO("%s: %d: ru->id %d ru->rg_id %d ru->ruh_id %d spp->pgs_per_ru %d ru->vpc %d ru->ipc %d blk->vpc %d line->vpc %d line->id %d ru->is_victim %d line %p ru_line %p\n", 
				__func__, io_type, ru->id, ru->rg_id, ru->ruh_id, spp->pgs_per_ru, ru->vpc, ru->ipc, blk->vpc, line->vpc, line->id, ru->is_victim, line, ru_line);
	}
	NVMEV_ASSERT(ru->vpc >= 0 && ru->vpc < spp->pgs_per_ru);
	ru->vpc++;

	rgm = get_rgm(fdp_ftl, ppa);
	rgm->vpc++;
}
...
{
	struct ssdparams *spp = &fdp_ftl->ssd->sp;
	struct line_mgmt *lm = &(fdp_ftl->ch_lm.lun_lm[ppa->g.ch].lm[ppa->g.lun]);
	...
	ru = get_ru(fdp_ftl, ppa);
	NVMEV_ASSERT(ru->ipc >= 0 && ru->ipc < spp->pgs_per_ru);
	// NVMEV_INFO("%s() ru->vpc %d spp->pgs_per_ru %d\n", __func__,ru->vpc, spp->pgs_per_ru);
	rgm = get_rgm(fdp_ftl, ppa);
	if (ru->vpc == spp->pgs_per_ru && ru->ref_cnt == 0) {
		NVMEV_ASSERT(ru->ipc == 0);
		NVMEV_ASSERT(ru->is_victim == 0);
		NVMEV_ASSERT(ru->pos == 0);
	...
	if (ru->pos) {
		if(ru->is_victim == 0) {
			NVMEV_INFO("%s: ru->id %d ru->rg_id %d ru->ruh_id %d spp->pgs_per_ru %d ru->vpc %d ru->ipc %d blk->vpc %d blk->ipc %d line->vpc %d line->ipc %d line->id %d\n", 
					__func__, ru->id, ru->rg_id, ru->ruh_id, spp->pgs_per_ru, ru->vpc, ru->ipc, blk->vpc, blk->ipc, line->vpc, line->ipc, line->id);
		}
	...
	if (ru->vpc != line->vpc || ru->vpc != blk->vpc ) {
		NVMEV_INFO("%s: ru->id %d ru->rg_id %d ru->ruh_id %d spp->pgs_per_ru %d ru->vpc %d ru->ipc %d blk->vpc %d blk->ipc %d line->vpc %d line->ipc %d line->id %d\n", 
				__func__, ru->id, ru->rg_id, ru->ruh_id, spp->pgs_per_ru, ru->vpc, ru->ipc, blk->vpc, blk->ipc, line->vpc, line->ipc, line->id);
	}
...
#ifdef FDP_SIMULATOR
/* move valid page data (already in DRAM) from victim line to a new page */
static uint64_t fdp_gc_write_page(struct fdp_ftl *fdp_ftl, uint32_t ruh_id, int rg_id, struct ppa *old_ppa)
{
	struct ssdparams *spp = &fdp_ftl->ssd->sp;
	struct fdpparams *fpp = &fdp_ftl->fp;
	struct ppa new_ppa;
	uint64_t lpn = fdp_get_rmap_ent(fdp_ftl, old_ppa);

	struct reclaim_unit_handle *ruh = __get_ftl_ruh(fdp_ftl, ruh_id);
	// int *ru_idx = __get_ruh_ru_idx(ruh, GC_IO);
	// *ru_idx = rg_id;


	NVMEV_ASSERT(valid_lpn((struct conv_ftl *) fdp_ftl, lpn));

	new_ppa = get_fdp_new_page(fdp_ftl, ruh_id, GC_IO);

	/* update maptbl */
	fdp_set_maptbl_ent(fdp_ftl, lpn, &new_ppa);
	/* update rmap */
	fdp_set_rmap_ent(fdp_ftl, lpn, &new_ppa);

	fdp_mark_page_valid(fdp_ftl, &new_ppa, 999, ruh_id, GC_IO);
...
static struct reclaim_unit *select_victim_ru(struct fdp_ftl *fdp_ftl, int gc_rg_idx, int phnd_id, bool force)
{
	...
	if (!force && (victim_ru->vpc > (spp->pgs_per_ru / 8))) {
		NVMEV_INFO("[NoFreeRU] %s() victim_ru->vpc  %d  pgs_per_ru/8 %d \n", 
				__func__, victim_ru->vpc, spp->pgs_per_ru / 8);
		return NULL;
	}

	if (victim_ru->is_victim == 0) {
		NVMEV_INFO("%s: why victim_ru inserted? ru->ruh_id %d ru->rg_id %d ru->id %d ru->ref_cnt %d ru->pos %d\n", 
				__func__, victim_ru->ruh_id, victim_ru->rg_id, victim_ru->id, victim_ru->ref_cnt, victim_ru->pos);
...
#ifdef FDP_SIMULATOR
/* here ppa identifies the block we want to clean */
static void fdp_clean_one_flashpg(struct fdp_ftl *fdp_ftl, uint32_t ruh_id, int rg_id, struct ppa *ppa)
{
	struct ssdparams *spp = &fdp_ftl->ssd->sp;
	struct fdpparams *fpp = &fdp_ftl->fp;
	struct nand_page *pg_iter = NULL;
	...
	for (i = 0; i < spp->pgs_per_flashpg; i++) {
		...
		if (pg_iter->status == PG_FREE) {
			...
			NVMEV_ERROR("%s: PG_FREE pg: %d blk: %d pl: %d lun: %d ch: %d ru->lun: %d ru->ch %d ru->id %d\n", 
					__func__, ppa_copy.g.pg, ppa_copy.g.blk, ppa_copy.g.pl, ppa_copy.g.lun, ppa_copy.g.ch, ru->lun, ru->ch, ru->id);
			NVMEV_ERROR("%s: ipc: %d vpc: %d\n", __func__, icnt, cnt);
		}
	...
	for (i = 0; i < spp->pgs_per_flashpg; i++) {
		...
		if (pg_iter->status == PG_VALID) {
			/* delay the maptbl update until "write" happens */
			fdp_gc_write_page(fdp_ftl, ruh_id, rg_id, &ppa_copy);
		}
...
static void mark_ru_free(struct fdp_ftl *fdp_ftl, struct reclaim_unit *ru)
{
...
static void mark_ru_line_free(struct fdp_ftl *fdp_ftl, struct line *line)
{
	struct reclaim_unit *ru = line->rup;
	struct line_mgmt *lm = &fdp_ftl->ch_lm.lun_lm[ru->ch].lm[ru->lun];
...
static int fdp_do_gc(struct fdp_ftl *fdp_ftl, int rg_id, int phnd_id, bool force)
{
	...
	if (!victim_ru) {
		NVMEV_INFO("%s: fdp_ftl %d rg_id: %d victim_ru is Null\n", 
				__func__, fdp_ftl->id, rg_id);
		return -1;
	}
...
#ifdef WAF
	fdp_ftl->ns->write_volume_gc += victim_ru->vpc;
	fdp_ftl->ns->total_write_volume_gc += victim_ru->vpc;
#endif //WAF
	...
	while (victim_ru->ulc > 0) {
		victim_line = list_first_entry_or_null(&victim_ru->ru_line_list, 
				struct line, entry);
		struct line_mgmt *lm = &fdp_ftl->ch_lm.lun_lm[victim_ru->ch].lm[victim_ru->lun];
		...
		for (flashpg = 0; flashpg < spp->flashpgs_per_blk; flashpg++) {
			...
			fdp_clean_one_flashpg(fdp_ftl, victim_ru->ruh_id, rg_id, &ppa);

			if (flashpg == (spp->flashpgs_per_blk - 1)) {
				struct fdpparams *fpp = &fdp_ftl->fp;

				fdp_mark_block_free(fdp_ftl, &ppa);
		...
		victim_ru->ulc --;
		/* update line status */
		mark_ru_line_free(fdp_ftl, victim_line);
	}

	mark_ru_free(fdp_ftl, victim_ru);
...
static void fdp_per_reclaim_group_gc(struct fdp_ftl *fdp_ftl, int phnd_id, int rg_id)
{
	if (fdp_should_gc(fdp_ftl, rg_id)) {
		NVMEV_DEBUG_VERBOSE("should_gc_high passed");
		/* perform GC here until !should_gc(conv_ftl) */
		fdp_do_gc(fdp_ftl, rg_id, phnd_id, false);
		//NVMEV_INFO("[NoFreeLine] %s <- do_gc() : %d\n", __func__, ret);
	}
}
...
static bool fdp_read(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	for (i = 0; (i < nr_parts) && (start_lpn <= end_lpn); i++, start_lpn++) {
		...
		xfer_size = 0;
		prev_ppa = fdp_get_maptbl_ent(fdp_ftl, start_lpn / nr_parts);
		...
		for (lpn = start_lpn; lpn <= end_lpn; lpn += nr_parts) {
			...
			local_lpn = lpn / nr_parts;
			cur_ppa = fdp_get_maptbl_ent(fdp_ftl, local_lpn);
...
static bool fdp_write(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	if (allocated_buf_size < LBA_TO_BYTE(nr_lba)) {
		// NVMEV_INFO("[FDP_SIMULATOR] %s() allocated buffer not enough\n", __func__);
		return false;
	}
	...
	/*
	if (dspec != 0) {
		NVMEV_INFO("[FDP_SIMULATOR] %s() dspec: %d\n", __func__, dspec);
		NVMEV_INFO("[FDP_SIMULATOR] %s() start_lpn=%lld, len=%lld, end_lpn=%lld nr_parts %d", 
	...
	*/
	...
	for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
		...
		local_lpn = lpn / nr_parts;
		ppa = fdp_get_maptbl_ent(fdp_ftl, local_lpn); // Check whether the given LPN has been written before
		if (mapped_ppa(&ppa)) {
			/* update old page information first */
			mark_placement_page_invalid(fdp_ftl, dspec, &ppa);
			fdp_set_rmap_ent(fdp_ftl, INVALID_LPN, &ppa);
			// NVMEV_INFO("%s: %lld is invalid ", __func__, fdp_ppa2pgidx(fdp_ftl, &ppa));
		}

		/* new write */
		ppa = get_fdp_new_page(fdp_ftl, dspec, USER_IO);
		/*
		NVMEV_INFO("%s: (fdp_ftl %d phnd %d ru_idx %d) local_lpn %d got new ppa %lld ", 
			__func__, fdp_ftl->id, dspec, ruh->ru_idx, local_lpn, fdp_ppa2pgidx(fdp_ftl, &ppa));
			*/

		/* update maptbl */
		fdp_set_maptbl_ent(fdp_ftl, local_lpn, &ppa);

		struct reclaim_unit_handle *ruh = __get_ftl_ruh(fdp_ftl, dspec);
		/*
		NVMEV_INFO("%s: (fdp_ftl %d phnd %d ru_idx %d) local_lpn %d got new pgidx %lld fdp_ftl->maptbl[%lld] %lld", 
			__func__, fdp_ftl->id, dspec, ruh->ru_idx, local_lpn, 
			fdp_ppa2pgidx(fdp_ftl, &ppa), local_lpn, fdp_ftl->maptbl[local_lpn]);
		*/


		/* update rmap */
		fdp_set_rmap_ent(fdp_ftl, local_lpn, &ppa);
		fdp_mark_page_valid(fdp_ftl, &ppa, lpn % nr_parts, dspec, USER_IO);

		/* need to advance the write pointer here */
		advance_fdp_write_pointer(fdp_ftl, dspec, USER_IO);

		/* Aggregate write io in flash page */
		if (fdp_last_pg_in_wordline(fdp_ftl, &ppa)) {
			swr.ppa = &ppa;
